...
def load_and_clean_merged_csv():
    ...
    features = features.dropna(subset=['credit_score', 'yearly_income', 'total_debt', 'amount']) # Dropna on relevant features
    ...
    df = df.merge(features[['credit_score', 'yearly_income', 'total_debt', 'amount', 'segment_label']],
                  ...
                  how='left', suffixes=('', '_y')) # Added suffix to avoid potential column name clashes
    
    # Drop the duplicate columns created by merge if any
    df.drop(columns=[col for col in df.columns if '_y' in col], inplace=True, errors='ignore')
...
def generate_advanced_kpi_and_charts(df):
    ...
    df_copy['txn_month'] = df_copy['txn_date'].dt.to_period("M").dt.to_timestamp()
    
    current_month = df_copy['txn_month'].max()
    # Check if there's a previous month to calculate MTD change
    if pd.notna(current_month):
        previous_month = current_month - pd.DateOffset(months=1)
    else:
        previous_month = None

    current_avg_limit = df_copy[df_copy['txn_month'] == current_month]['credit_limit'].mean()
    previous_avg_limit = df_copy[df_copy['txn_month'] == previous_month]['credit_limit'].mean() if previous_month else 0
...
if not st.session_state['authenticated']:
    ...
    if selected == "Ana Sayfa":
        ...
        if 'segment_label' in df.columns:
            ...
            st.warning("Segmentasyon verisi bulunamadı. Lütfen veri yükleme ve segmentasyon adımlarını kontrol edin.")
        ...
        if 'segment_label' in df.columns:
            ...
            st.info("Segment dağılımı grafiği için segmentasyon verisi eksik.")
